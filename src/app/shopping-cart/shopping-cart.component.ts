...
export class ShoppingCartComponent implements OnInit, OnDestroy {
  ...
  fetchShoppingCart() {
    this.totalPrice = 0;
    this.store.dispatch(
      new ShoppingCartActions.FetchShoppingCart(+this.user.userId)
    );
  }
  ...
  onEndDeleteShoppingCartItemAnimation() {
    if (this.shoppingCartItem !== null) {
      ...
      this.deleteShoppingCartItem(
        +this.user.userId,
        +this.shoppingCartItem.products.id
      );
  ...
  deleteShoppingCartItem(userId: number, productId: number) {
    this.store.dispatch(
      new ShoppingCartActions.DeleteCartItem({
        userId: userId,
        productId: productId,
      })
    );
  ...
  deleteAllShoppingCartItems() {
    this.store.dispatch(
      new ShoppingCartActions.DeleteAllCartItems(+this.user.userId)
    );
  }
